// Slightly better than V7 this will be kept
...
int cost_func(const Sequence_vec& Sequence, const distances& dist_array, int n, int max_rows) {
    int dist = 0;
    int max_storage = 25;
    ...
    dist += dist_array[n-1][Sequence[0].idx];
    idx1 = Sequence[0].idx;
    ...
    for (int i = 1; i < n; i++) {
        ...
        dist += dist_array[idx1][idx2] + over_wei;
        idx1 = idx2;

    }
    dist += dist_array[n-1][Sequence[n-1].idx];

    return dist;
}

void calc_dist_array(const Sequence_vec& Sequence, distances& dist_array, int n, int max_rows) {
    for (int i = 0; i < n; ++i) {
        int x1 = Sequence[i].x;
        int y1 = Sequence[i].y;
        for (int j = i + 1; j < n; ++j) {
            int x2 = Sequence[j].x;
            int y2 = Sequence[j].y;
            int d = distance_func(x1, y1, x2, y2, max_rows);
            dist_array[i][j] = d;
            dist_array[j][i] = d;
        }
    }
...
SAResults sa_iteration(const Sequence_vec& initial_sequence, distances& dist_array, int Lk, float c, std::mt19937& g, int n, int max_rows) {
    SAResults results;
...
float initial_temp(const Sequence_vec& Sequence, distances& dist_array, int n, int seed, int max_rows, int Lk, float ratio) {
...
SARuns SA(Sequence_vec& Sequence, distances& dist_array, int n, int seed, int max_rows, int max_cols, float c_in, int Lk, float cooling_rate, int freeze_crit) {
    SARuns run;
    run.seed = seed;
...
int main() {
    ...
    distances dist_array(n, std::vector<int>(n, 0));
