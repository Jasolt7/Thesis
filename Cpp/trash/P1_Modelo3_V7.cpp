...
SARuns run_sa_with_seed(const delta_vec& delta, const int* rec_max_p, const int* pro_i, 
                       const int* durations, int T, int n, int seed, float c_in, 
                       int Lk, float cooling_rate, int freeze_crit) {
    return SA(delta, rec_max_p, pro_i, durations, T, n, seed, c_in, Lk, cooling_rate, freeze_crit);
}
