// valgrind --tool=callgrind --simulate-cache=yes ./P1_Modelo3_V4
...
int earliest_candidate(RU_vec& RU_define_sequence, const delta_vec& delta, const int* rec_max_p, const int* durations, int i, int start, int end) {
    int pri1 = 8;
    int pri2 = 16;
...
std::vector<int> define_sequence_LS(const Sequence_vec& Sequence, const delta_vec& delta, RU_vec RU_define_sequence, int T, int n, const int* rec_max_p, const int* pro_i, const int* durations) {
    std::vector<int> S(n, 0);
    for (int i = 0; i < n; i++) {
        int exam = Sequence[i];
        int exam_type = pro_i[exam];
        int t = earliest_candidate(RU_define_sequence, delta, rec_max_p, durations, exam_type, 0, T);
        if (t < 0) {
...
SAResults sa_iteration(const delta_vec& delta, int T, int n, const int* rec_max_p, const int* pro_i, const int* durations, int Lk, float c, const Sequence_vec& initial_sequence, std::mt19937& g) {
    ...
    std::vector<int> S_best = define_sequence_LS(results.Sequence_best, delta, RU_define_sequence, T, n, rec_max_p, pro_i, durations);
    ...
    for (int it = 0; it < Lk; it++) {
        ...
        auto S = define_sequence_LS(results.Sequence, delta, RU_define_sequence, T, n, rec_max_p, pro_i, durations);
...
int main() {
    ...
    SAResults result = sa_iteration(delta, T, n, rec_max_p, pro_i, durations, 1000, c_in, Sequence, g);
    ...
    RU_vec RU_define_sequence(T, std::vector<int>(max_rows-2, 0));
    std::vector<int> S_best = define_sequence_LS(result.Sequence_best, delta, RU_define_sequence, T, n, rec_max_p, pro_i, durations);
    int best_cost = compute_cost(S_best, delta, n, pro_i);
    for (int i = 0; i < n; ++i) {
        printf("%d,", S_best[i]);
    }
    ...
    for (int times = 0; times < 1000; times++) {
        ...
        S_best = define_sequence_LS(result.Sequence_best, delta, RU_define_sequence, T, n, rec_max_p, pro_i, durations);
