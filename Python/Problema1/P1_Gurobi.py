# -*- coding: utf-8 -*-
"""
Created on Wed Jun 18 09:39:23 2025
...
F = model.addVars(n, lb=0, ub=T, vtype=GRB.INTEGER, name="F")   # continuous or integer
Cmax = model.addVar(lb=0, ub=T, vtype=GRB.INTEGER, name="Cmax")
...
for i in range(n):
    ...
    dur   = delta.shape[0]
    model.addConstr(
        quicksum(Z[i, t] for t in range(T-dur+1)) == 1)
...
for i in range(n):
    ...
    dur   = delta.shape[0]
    model.addConstr(
        quicksum(Z[i, t]*(t+dur) for t in range(T-dur+1)) == F[i])

for i in range(n):
    model.addConstr(F[i] <= Cmax)
...
figplot(S_ik, F_ik, n, int(Cmax.X), max_cols)
plot_resource_utilization(rec_max_p, int(Cmax.X), resource_usage, rec_lpk, pro_i)