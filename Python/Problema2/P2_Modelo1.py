# -*- coding: utf-8 -*-
"""
Created on Sun Sep  7 14:07:18 2025
...
def sa_iteration(Y_i, S_ik, F_ik, resource_usage, c, pro_time, pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, Lk, punish, nei_1, nei_2):
    ...
    for it in range(Lk):
        ...
        #shift
        if neighbour == 0:
            assigned = get_array(Y_temp, 1)
            if len(assigned) == 0:
                neighbour = -1
                continue
            i = assigned[int(len(assigned) * i_it[it])]
            # Get the current boundaries for exam i.
            original_start = int(S_temp[i, 0])
            original_finish = int(F_temp[i, -1])
            
            # Choose a shift so that the exam remains within [0, T].
            #shift = -original_start + (makespan - original_finish + 1 - -original_start) * shift_it[it]
            shift = int(-original_start + (T - original_finish + 1 - -original_start) * shift_it[it])
            ...
            # Remove exam i from its current location.
        ...
        #add  
            ...
            unassigned = get_array(Y_temp, 0)
            if len(unassigned) == 0:
                neighbour = -1
                continue
            i = unassigned[int(len(unassigned) * i_it[it])]
            
            exam_type = pro_i[i]
            delta = exam_type_deltas[exam_type]
            
            shift = int((T - delta.shape[0] + 1) * shift_it[it])
            
            S_temp[i, 0] = shift
            F_temp[i, 0] = S_temp[i, 0] + rec_lpk[pro_i[i], 1, 0]
                
            for k in range(1, max_cols):
                S_temp[i, k] = F_temp[i, k-1]
                F_temp[i, k] = S_temp[i, k] + rec_lpk[pro_i[i], 1, k]
            
            # Insert the exam pattern into the new location.
            original_start = int(S_temp[i, 0])
            original_finish = original_start + delta.shape[0]
...
def initial_temp(pro_time, pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, punish, ratio, nei_1, nei_2):
    ...
    Lk = 100000
    _, _, _, _, acc, _, _, _, _, _, _, average_dif = sa_iteration(Y_i, S_ik, F_ik, resource_usage, float("inf"), pro_time, pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, Lk, punish, nei_1, nei_2)
...
def SA(pro_time, pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, c_in, seed, Lk, freeze_crit, temp_red, punish, ratio, nei_1, nei_2):
    ...
    for i in range(len(Y_i)):
        if Y_i[i] == 1:
            ...
            resource_usage[t : t + delta.shape[0], :] += delta
...
#_, _, _, _, _, _ = main(100, 10, 50, 1000, 150, 0.8, 1, 0.7, 0.05, 0.05)

for Lk in [1000, 3000, 5000]:
    for freeze_crit in [30, 90, 150]:
        for temp_red in [0.8, 0.9, 0.975]:
            for punish in [1, 5, 10]:
