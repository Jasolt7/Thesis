"""
Created on Sun Oct  5 14:05:15 2025
...
def define_sequence_ELS(Sequence, threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, start, end):
    ...
    for i in Sequence:
        ...
        duration_glob = sum(rec_lpk[pro_i[i],1])
        if exam_type_start[exam_type] <= end:
            t = earliest_candidate(resource_usage_1, delta, threshold, start, end, pri_s_cam_gama, pri_s_tom)
            if t == -1:
                t = T - duration_glob - 1
            if t > exam_type_start[exam_type]:
                exam_type_start[exam_type] = t
    ...
    makespan_1 = -np.count_nonzero(F_ik_1[:, -1] <= end)
    ...
    for i in Sequence_alt:
        ...
        duration_glob = sum(rec_lpk[pro_i[i],1])
        if exam_type_start[exam_type] <= end:
            t = earliest_candidate(resource_usage_2, delta, threshold, start, end, pri_s_cam_gama, pri_s_tom)
            if t == -1:
                t = T - duration_glob - 1
            if t > exam_type_start[exam_type]:
                exam_type_start[exam_type] = t
    ...
    makespan_2 = -np.count_nonzero(F_ik_2[:, -1] <= end)
...
def initial_solution(threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, final_time):
    order = np.zeros(n)
    for i in range(n):
        exam_type = pro_i[i]
        delta = exam_type_deltas[exam_type]
    ...
    for step in range(2, len(pro_i)):
        ...
        for l in range(Sequence_size):
            ...
            _, F_alt, _, _ = define_sequence_LS(Sequence_alt, threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, 0, final_time)
            alt_cost = -np.count_nonzero(F_alt[:, -1] <= final_time)
...
def sa_iteration(Sequence, c, pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, Lk, punish, final_time):
    Sequence_best = deep_copy(Sequence)
    S_ik, F_ik, resource_usage, makespan = define_sequence_LS(Sequence, threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, 0, final_time)
    current_cost = -np.count_nonzero(F_ik[:, -1] <= final_time)
    best_cost = -np.count_nonzero(F_ik[:, -1] <= final_time)
    ...
    for it in range(Lk):
        ...
        S_ik, F_ik, resource_usage, makespan = define_sequence_LS(Sequence, threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, 0, final_time)
        #print(makespan)
        alt_cost = -np.count_nonzero(F_ik[:, -1] <= final_time)
...
def initial_temp(pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, punish, ratio, final_time):
    np.random.seed(seed)

    Sequence = initial_solution(threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, final_time)
    
    Lk = 1000
    _, _, acc, _, _, _, _, _, average_dif = sa_iteration(Sequence, float("inf"), pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, Lk, punish, final_time)
    c_in = np.abs((average_dif/Lk)/np.log(ratio))
    return c_in

def SA(pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, c_in, seed, Lk, freeze_crit, temp_red, punish, ratio, final_time):
    np.random.seed(seed)
    ...
    while freeze < freeze_crit:
        Sequence, _, _, newSequence, _, _, _, _, _ = sa_iteration(Sequence, c, pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, Lk, punish, final_time)
        newS, newF, newRU, makespan = define_sequence_LS(newSequence, threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, 0, final_time)
        new_cost = -np.count_nonzero(newF[:, -1] <= final_time)
...
def main(n_runs, n_threads, Lk, freeze_crit, temp_red, punish, ratio, final_time, pro_i):    
    seed = 50
    ...
    c_in = initial_temp(pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, seed, punish, ratio, final_time)
    print("c_in", c_in)
    
    start_time_main = time.time()
    results = Parallel(n_jobs=n_threads)(delayed(SA)(pro_i, rec_lpk, rec_max_p, n, n_exams, max_cols, max_rows, T, R, threshold, exam_type_deltas, c_in, seed, Lk, freeze_crit, temp_red, punish, ratio, final_time) for seed in range(n_runs))
    best_cost = float('inf')
    ...
    for result in results:
        ...
        S_alt, F_alt, RU_alt, makespan = define_sequence_LS(Sequcne_alt, threshold, T, pro_i, rec_lpk, n, max_cols, exam_type_deltas, 0, final_time)
        cost_alt = -np.count_nonzero(F_alt[:, -1] <= final_time)
    ...
    cost = -np.count_nonzero(F_ik[:, -1] <= final_time)
    ...
    figplot(S_ik, F_ik, n, final_time, max_cols)
    plot_resource_utilization(rec_max_p, final_time, resource_usage, rec_lpk, pro_i)
    
    return Sequence, S_ik, F_ik, cost, time.time() - start_time_main, resource_usage

_, _, _, _, _, _ = main(10, 1, 250, 55, 0.975, 0, 0.5, 487, np.array([3, 5, 10, 1, 10])*2)
        ...
        for temp_red in [0.8, 0.9, 0.975]:
            for punish in [0]:
                for ratio in [0.5, 0.7, 0.9]:
                    ...
                    _, _, _, _, _, _ = main(100, 10, Lk, freeze_crit, temp_red, punish, ratio, final_time, np.array([3, 5, 10, 1, 10])*2)
                    print("\n")
"""
