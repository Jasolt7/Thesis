# -*- coding: utf-8 -*-
"""
Created on Thu Oct 16 23:15:31 2025
...
def calc_dist_array(Sequence, max_rows):
    n = len(Sequence)
    dist_array = np.zeros((n, n))+999
    for i in range(n):
        p1 = Sequence[i][:2]
        for j in range(i+1, n):
            p2 = Sequence[j][:2]
            d = distance_func(p1, p2, max_rows)
            dist_array[i][j] = d
            dist_array[j][i] = d
    return dist_array

def cost_func1(Sequence, max_rows):
    dist = 0
    max_storage = 25
    current_storage = 0
    over_wight = 0
    ...
    for seq in Sequence:
        ...
        if current_storage > max_storage:
            over_wight = current_storage - max_storage
        else:
            over_wight = 0
        dist += get_distance(p1, p2, max_rows) + 10*(over_wight)
        print(p1,p2,get_distance(p1, p2, max_rows),10*(over_wight),current_storage)
...
def cost_func2(Sequence, dist_array, max_rows):
    n = len(Sequence)
    dist = 0
    max_storage = 25
    current_storage = 0
    ...
    dist += dist_array[n-1][Sequence[0][3]]
    print((0,0,0),Sequence[0][:3],dist_array[n-1][Sequence[0][3]],10*(over_wight),current_storage)
    for i in range(1, n):
        p1 = Sequence[i-1][:2]
        idx1 = Sequence[i-1][3]
        p2 = Sequence[i][:2]
        ...
        if current_storage > max_storage:
            ...
            over_wight = 0
        dist += dist_array[idx1][idx2] + 10*(over_wight)
        print(Sequence[i-1][:3],Sequence[i][:3],dist_array[idx1][idx2],10*(over_wight),current_storage)
    
    dist += dist_array[n-1][Sequence[n-1][3]]
